...
# ==============================================================================
# HÀM NHẬN DIỆN LOẠI BẢNG KÊ (VIẾT LẠI)
...
def detect_report_type(file_content):
    ...
    Nếu đọc được header của HDDT -> là file HDDT.
    """
    ...
    try:
        ...
        # Nếu không đọc được, bỏ qua và thử cách của HDDT
        pass
    ...
    try:
        ...
        # Các cột đặc trưng của file HDDT
        ...
        # Nếu vẫn không được, coi như không nhận diện được
        pass
...
def process_pos_report(file_content, selected_chxd, price_periods, new_price_invoice_number, **kwargs):
    ...
    def _create_pos_invoice_row(row, final_date, details, static_data):
        ...
        new_row[17] = format_tax_code(_to_float_pos(row['Thuế GTGT']))
    ...
    if price_periods == '1':
        ...
        wb_old, wb_new = _create_upsse_workbook(), _create_upsse_workbook()
...
def process_unified_file(file_content, selected_chxd, price_periods, new_price_invoice_number, confirmed_date_str=None):
    ...
    # Truyền file content dạng bytes vào các hàm xử lý
    if report_type == 'POS':
