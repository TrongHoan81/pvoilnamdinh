...
def _format_tax_code_shared(raw_vat_value):
...
def process_pos_report(file_content_bytes, selected_chxd, price_periods, new_price_invoice_number, **kwargs):
    ...
    def _get_static_data_pos(file_path):
        try:
            ...
            for row_idx in range(4, ws.max_row + 1):
                ...
                chxd_name = _clean_string_shared(row_values[3])
                if not chxd_name: continue
                chxd_detail_map[chxd_name] = {
                    'g4_val': _clean_string_shared(row_values[6]), 'g5_val': _clean_string_shared(row_values[4]),
                    't_val': _clean_string_shared(row_values[19]), 'u_val': _clean_string_shared(row_values[20]),
                    'v_val': _clean_string_shared(row_values[21]), 'w_val': _clean_string_shared(row_values[22]),
                    'x_val': _clean_string_shared(row_values[23])
                }
                store_specific_x_lookup[chxd_name] = {
                    _clean_string_shared(ws['O3'].value).lower(): _clean_string_shared(row_values[14]),
                    _clean_string_shared(ws['P3'].value).lower(): _clean_string_shared(row_values[15]),
                    _clean_string_shared(ws['Q3'].value).lower(): _clean_string_shared(row_values[16]),
                    _clean_string_shared(ws['R3'].value).lower(): _clean_string_shared(row_values[17])
                }
            tmt_map = {_clean_string_shared(ws[f'A{i}'].value): _to_float_shared(ws[f'B{i}'].value) for i in range(4, 8)}
    ...
    def _create_pos_summary_row(product_name, data, final_date, details, static_data, is_new_price, selected_chxd):
        ...
        new_row[17] = _format_tax_code_shared(data['thue'] * 100)
        new_row[18], new_row[19], new_row[20], new_row[21] = details['t_val'], details['u_val'], details['v_val'], details['w_val']
        new_row[31] = f"Khách mua {product_name} không lấy hóa đơn"
        new_row[36] = data['tienthuegtgt'] - round(tmt_value * data['sl'] * (_to_float_shared(new_row[17]) / 100.0), 0)
        return new_row

    def _create_pos_tmt_row(original_row, tmt_value, details):
        tmt_row = list(original_row)
        tax_rate_decimal = _to_float_shared(original_row[17]) / 100.0
        tmt_row[6], tmt_row[7], tmt_row[8] = "TMT", "Thuế bảo vệ môi trường", "Lít"
        tmt_row[9] = details['g5_val']
        tmt_row[13] = tmt_value
        tmt_row[14] = round(tmt_value * _to_float_shared(original_row[12]), 0)
...
def process_hddt_report(file_content_bytes, selected_chxd, price_periods, new_price_invoice_number, confirmed_date_str=None):
    ...
    def _load_static_data_hddt(data_file_path, mahh_file_path, dskh_file_path):
        try:
            ...
            vu_viec_map = {}
            vu_viec_headers = [_clean_string_shared(cell.value) for cell in ws[2][4:9]]
            for row_values in ws.iter_rows(min_row=3, max_col=12, values_only=True):
                chxd_name = _clean_string_shared(row_values[3])
                if chxd_name:
                    ma_kho, khhd, khu_vuc = _clean_string_shared(row_values[9]), _clean_string_shared(row_values[10]), _clean_string_shared(row_values[11])
                    ...
                    for i, header in enumerate(vu_viec_headers):
                        if header:
                            ...
                            vu_viec_map[chxd_name][key] = _clean_string_shared(vu_viec_data_row[i])
            ...
            def get_lookup_map(min_r, max_r, min_c=1, max_c=2):
                return {_clean_string_shared(row[0]): row[1] for row in ws.iter_rows(min_row=min_r, max_row=max_r, min_col=min_c, max_col=max_c, values_only=True) if row[0] and row[1] is not None}
            phi_bvmt_map_raw = get_lookup_map(10, 13)
            static_data["phi_bvmt_map"] = {_clean_string_shared(k): _to_float_shared(v) for k, v in phi_bvmt_map_raw.items()}
            static_data.update({
            ...
            static_data["ma_hang_map"] = {_clean_string_shared(r[0]): _clean_string_shared(r[2]) for r in wb_mahh.active.iter_rows(min_row=2, max_col=3, values_only=True) if r[0] and r[2]}
            wb_dskh = load_workbook(dskh_file_path, data_only=True)
            static_data["mst_to_makh_map"] = {_clean_string_shared(r[2]): _clean_string_shared(r[3]) for r in wb_dskh.active.iter_rows(min_row=2, max_col=4, values_only=True) if r[2]}
    ...
    def _create_hddt_bvmt_row(original_row, phi_bvmt, static_data, khu_vuc):
        ...
        so_luong = _to_float_shared(original_row[12])
        thue_suat = _to_float_shared(original_row[17]) / 100.0 if original_row[17] else 0.0
    ...
    def _generate_upsse_from_hddt_rows(rows_to_process, static_data, selected_chxd, final_date, summary_suffix_map):
        ...
        for bkhd_row in rows_to_process:
            if _to_float_shared(bkhd_row[8] if len(bkhd_row) > 8 else None) <= 0: continue
            ten_kh, ten_mat_hang = _clean_string_shared(bkhd_row[3]), _clean_string_shared(bkhd_row[6])
            ...
            if not is_anonymous or not is_petrol:
                ...
                new_upsse_row[4] = _clean_string_shared(bkhd_row[17]) + _clean_string_shared(bkhd_row[18])
                ...
                new_upsse_row[8], new_upsse_row[12] = _clean_string_shared(bkhd_row[10]), round(_to_float_shared(bkhd_row[8]), 3)
                phi_bvmt = static_data['phi_bvmt_map'].get(ten_mat_hang, 0.0) if is_petrol else 0.0
                new_upsse_row[13] = _to_float_shared(bkhd_row[9]) - phi_bvmt
                ma_thue = _format_tax_code_shared(bkhd_row[14])
                new_upsse_row[17] = ma_thue
                thue_suat = _to_float_shared(ma_thue) / 100.0 if ma_thue else 0.0
                tien_thue_goc, so_luong = _to_float_shared(bkhd_row[15]), _to_float_shared(bkhd_row[8])
                tien_thue_phi_bvmt = round(phi_bvmt * so_luong * thue_suat)
                new_upsse_row[36] = round(tien_thue_goc - tien_thue_phi_bvmt)
                new_upsse_row[14] = round(_to_float_shared(bkhd_row[13]) if not is_petrol else _to_float_shared(bkhd_row[16]) - tien_thue_goc - round(phi_bvmt * so_luong))
                ...
                new_upsse_row[32], mst_khach_hang = _clean_string_shared(bkhd_row[4]), _clean_string_shared(bkhd_row[5])
                new_upsse_row[33] = mst_khach_hang
                ma_kh_fast = _clean_string_shared(bkhd_row[2])
                ...
                if ten_mat_hang not in summary_data:
                    summary_data[ten_mat_hang] = {'sl': 0, 'thue': 0, 'phai_thu': 0, 'first_data': {'mau_so': _clean_string_shared(bkhd_row[17]),'ky_hieu': _clean_string_shared(bkhd_row[18]),'don_gia': _to_float_shared(bkhd_row[9]),'vat_raw': bkhd_row[14]}}
                summary_data[ten_mat_hang]['sl'] += _to_float_shared(bkhd_row[8])
                summary_data[ten_mat_hang]['thue'] += _to_float_shared(bkhd_row[15])
                summary_data[ten_mat_hang]['phai_thu'] += _to_float_shared(bkhd_row[16])
        ...
        for product, data in summary_data.items():
            ...
            ma_thue = _format_tax_code_shared(first_data['vat_raw'])
            thue_suat = _to_float_shared(ma_thue) / 100.0 if ma_thue else 0.0
